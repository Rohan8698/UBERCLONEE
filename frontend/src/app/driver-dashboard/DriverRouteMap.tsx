import { useEffect, useRef } from "react";
import L from "leaflet";
import "leaflet/dist/leaflet.css";

const defaultCenter: [number, number] = [28.6139, 77.2090]; // New Delhi

export default function DriverRouteMap({ pickup, dropoff }: { pickup: string; dropoff: string }) {
  const mapRef = useRef<HTMLDivElement>(null);
  const leafletMapRef = useRef<L.Map | null>(null);
  const routeLineRef = useRef<L.Polyline | null>(null);

  // Geocode address to lat/lng using GraphHopper
  const geocode = async (query: string): Promise<[number, number] | null> => {
    const apiKey = "ac8bb813-de86-49fd-8a86-9edbc8798534";
    const url = `https://graphhopper.com/api/1/geocode?q=${encodeURIComponent(query)}&limit=1&key=${apiKey}`;
    const res = await fetch(url);
    const data = await res.json();
    if (data.hits && data.hits.length > 0) {
      return [data.hits[0].point.lat, data.hits[0].point.lng] as [number, number];
    }
    return null;
  };

  // Get route from GraphHopper
  const getRoute = async (from: [number, number], to: [number, number]) => {
    const apiKey = "ac8bb813-de86-49fd-8a86-9edbc8798534";
    const url = `https://graphhopper.com/api/1/route?point=${from[0]},${from[1]}&point=${to[0]},${to[1]}&vehicle=car&locale=en&points_encoded=false&key=${apiKey}`;
    const res = await fetch(url);
    const data = await res.json();
    if (data.paths && data.paths.length > 0) {
      const coords = data.paths[0].points.coordinates.map((c: [number, number]) => [c[1], c[0]]);
      return coords;
    }
    return null;
  };

  useEffect(() => {
    if (mapRef.current && !leafletMapRef.current) {
      leafletMapRef.current = L.map(mapRef.current).setView(defaultCenter, 12);
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; OpenStreetMap contributors',
      }).addTo(leafletMapRef.current);
    }
    return () => {
      if (leafletMapRef.current) {
        leafletMapRef.current.remove();
        leafletMapRef.current = null;
      }
    };
  }, []);

  useEffect(() => {
    let cancelled = false;
    const map = leafletMapRef.current;
    if (!map) return;
    // Remove old markers and route
    map.eachLayer(layer => {
      if ((layer as any).options && (layer as any).options.pane === "markerPane") {
        map.removeLayer(layer);
      }
    });
    if (routeLineRef.current) {
      map.removeLayer(routeLineRef.current);
      routeLineRef.current = null;
    }
    let pickupPos: [number, number] | null = null;
    let dropoffPos: [number, number] | null = null;
    (async () => {
      if (pickup && dropoff) {
        pickupPos = await geocode(pickup);
        dropoffPos = await geocode(dropoff);
        // Check map is still valid and not cancelled
        const currentMap = leafletMapRef.current;
        if (cancelled || !currentMap) return;
        if (pickupPos) L.marker([pickupPos[0], pickupPos[1]], { title: "Pickup" }).addTo(currentMap);
        if (dropoffPos) L.marker([dropoffPos[0], dropoffPos[1]], { title: "Dropoff" }).addTo(currentMap);
        if (pickupPos && dropoffPos) {
          const coords = await getRoute(pickupPos, dropoffPos);
          if (cancelled || !leafletMapRef.current) return;
          if (coords) {
            const poly = L.polyline(coords, { color: "blue" }).addTo(leafletMapRef.current);
            routeLineRef.current = poly;
            leafletMapRef.current.fitBounds(poly.getBounds(), { padding: [40, 40] });
          }
        }
      }
    })();
    return () => { cancelled = true; };
  }, [pickup, dropoff]);

  return <div ref={mapRef} style={{ width: "100%", height: 350, borderRadius: 8 }} />;
}
